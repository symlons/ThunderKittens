...
__global__ __launch_bounds__(256, 2) void layernorm_adaln_backward_k4096_vec2_kernel(
    bf16 *__restrict__ dx,
    const bf16 *__restrict__ grad,
    const bf16 *__restrict__ x,
    const bf16 *__restrict__ scale,
    ...
    constexpr int K2 = K / 2;
    using bf16_2 = __nv_bfloat162;
    __shared__ float2 warp_sums[8];
    __shared__ float2 total_sums;
    ...
    for (int j = 0; j < 8; j++) {
        ...
        float dnorm_x = gv.x * (1.0f + sc.x);
        float dnorm_y = gv.y * (1.0f + sc.y);
        sums.x += dnorm_x + dnorm_y;
        sums.y += dnorm_x * xhat_x + dnorm_y * xhat_y;
    }
    sums.x = warp_sum_float(sums.x);
    sums.y = warp_sum_float(sums.y);
    if (lane == 0) {
        warp_sums[warp] = sums;
    }
    __syncthreads();

    sums = tid < 8 ? warp_sums[lane] : make_float2(0.0f, 0.0f);
    if (warp == 0) {
        sums.x = warp_sum_float(sums.x);
        sums.y = warp_sum_float(sums.y);
    }
    ...
    __syncthreads();
    float s1 = total_sums.x;
    float s2 = total_sums.y;
...
__global__ __launch_bounds__(128, 4) void layernorm_adaln_param_backward_tok16_cols128_kernel(
    float *__restrict__ dshift,
    float *__restrict__ dscale,
    const bf16 *__restrict__ grad,
    const bf16 *__restrict__ x,
    ...
    dshift[batch * K + col] = shift_acc;
    dscale[batch * K + col] = scale_acc;
}
...
struct matmul_template {
    ...
    template<bool PERISISTENT_GRID=true> __host__ static inline dim3 grid(int M, int N, int K) {
        int full_grid = M*N/(M_BLOCK*N_BLOCK*layout::base_tile::num_elements);
        return dim3((PERISISTENT_GRID && M != 65536) ? PERSISTENT_GRID_BLOCKS : full_grid);
    }
...
struct native_matmul_template {
    ...
    template<bool PERISISTENT_GRID=true> __host__ static inline dim3 grid(int M, int N, int K) {
        int full_grid = M*N/(M_BLOCK*N_BLOCK*layout::base_tile::num_elements);
        return dim3((PERISISTENT_GRID && M != 65536) ? ((M >= 81920) ? 132 : PERSISTENT_GRID_BLOCKS) : full_grid);
    }
    __device__ static inline void common_setup(common_setup_args<layout> args) {
        if (threadIdx.x == 0) {
            args.globals.A.template prefetch_tma<typename layout::base_tile>();
...
template<int _M_BLOCK=2, int _N_BLOCK=4, int _SUPER_M=12>
struct native_matmul_gelu_out_template {
    static constexpr int M_BLOCK = _M_BLOCK, N_BLOCK = _N_BLOCK, SUPER_M = _SUPER_M;
    using layout    = matmul_layout<M_BLOCK, N_BLOCK>;
    using wide_tile = st_bf<64, 64*N_BLOCK>;
    static constexpr int NUM_CONSUMER_WARPS=M_BLOCK*4, INPUT_PIPE_STAGES=4, PRODUCER_BARRIER_ARRIVALS=1;
...
struct native_linear_grid132_template : native_linear_template<_M_BLOCK, _N_BLOCK, _SUPER_M> {
    ...
    template<bool PERISISTENT_GRID=true> __host__ static inline dim3 grid(int M, int N, int K) {
        int full_grid = M*N/(M_BLOCK*N_BLOCK*layout::base_tile::num_elements);
        return dim3((PERISISTENT_GRID && M != 65536) ? 132 : full_grid);
    }
};
...
template<int M_BLOCK, int N_BLOCK, int SUPER_M>
void gemm_custom_native_out_variant_entrypoint(
    const at::Tensor &A,
    const at::Tensor &B,
    const at::Tensor &C,
    const at::Tensor &bias
) {
    using mmt = native_matmul_gelu_out_template<M_BLOCK, N_BLOCK, SUPER_M>;
    using globals = typename mmt::layout::globals;
    using global_layout = typename mmt::layout::global_layout;
    using bias_global = typename mmt::layout::bias_global;
...
void gemm_custom_native_out_entrypoint(
    const at::Tensor &A,
    const at::Tensor &B,
    const at::Tensor &C,
    const at::Tensor &bias
    ...
    int N = B.size(0);
    if (K == 1024 && N == 4096) {
        gemm_custom_native_out_variant_entrypoint<2, 4, 4>(A, B, C, bias);
        return;
    }
    gemm_custom_native_out_variant_entrypoint<2, 4, 8>(A, B, C, bias);
}
...
void gemm_linear_native_entrypoint(
    ...
    if (K == 1024 && N == 3072) {
        if (M == 32768 || M == 65536 || M == 131072 || M == 262144) {
            gemm_linear_native_grid132_variant_entrypoint<2, 4, 4>(A, B, C, bias);
            return;
        }
        if (M == 393216) {
...
void layernorm_adaln_backward_entrypoint(
    ...
    if (K == 1024) {
        ...
        layernorm_adaln_backward_k4096_vec2_kernel<<<M, 256, 0, stream>>>(
            reinterpret_cast<bf16*>(dA.data_ptr()),
            reinterpret_cast<const bf16*>(grad.data_ptr()),
            reinterpret_cast<const bf16*>(A.data_ptr()),
            reinterpret_cast<const bf16*>(scale.data_ptr()),
    ...
    if (tokens_per_sample == 16) {
        dim3 param_block(128);
        dim3 param_grid((K + param_block.x - 1) / param_block.x, batch);
        layernorm_adaln_param_backward_tok16_cols128_kernel<<<param_grid, param_block, 0, stream>>>(
            reinterpret_cast<float*>(dshift.data_ptr()),
            reinterpret_cast<float*>(dscale.data_ptr()),
            reinterpret_cast<const bf16*>(grad.data_ptr()),
            reinterpret_cast<const bf16*>(A.data_ptr()),
...
PYBIND11_MODULE(_C, m) {
    ...
    m.def("gemm_custom_native_out", &gemm_custom_native_out_entrypoint);
